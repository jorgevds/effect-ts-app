...
export const CreateChore = () => {
    const [formState, formAction] = useFormState(createChoreSA, { status: "initial" as FormStateStatus, choreId: "", error: "" });
    ...
    return (
        <>
            <form action={formAction} className="max-w-sm pt-8">
                ...
                <div className="max-w-sm pb-8">
                    <label htmlFor="estimatedTime" className="block text-sm font-medium mb-2 ">
                        How often should this happen? (days)
                    </label>
                    <input
                        type="datetime-local"
                        id="timeInvalid"
                        name="timeInvalid"
                        value={formValues.timeInvalid}
                        onChange={event => setFormValues(state => ({ ...state, timeInvalid: event.target.value }))}
                        className="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none "
                        placeholder="7"
                    />
                </div>
                ...
                <div className="w-full flex justify-between">
                    <button
                        ...
                        disabled={!formValues.chore || !formValues.location || !formValues.estimatedTime || !formValues.timeInvalid}
                    >
                        Create
                    </button>
            ...
            <FormToast
                formState={formState}
                messages={{
                    success: "Successfully added a new chore",
